...
TiledMap::TiledMap(std::string name, int width, int height, int tileWidth, int tileHeight)
{
	...
	this->position = base::Vector2::Zero();
}

base::Vector2 TiledMap::GetPosition()
{
	return position;
}

void TiledMap::SetPosition(base::Vector2 position)
{
...
void TiledMap::Draw(SpriteExtensions& spriteBatch)
{
	...
	for (auto row = startRow; row <= endRow; row++)
	{
		for (auto column = startColumn; column <= endColumn; column++)
		{
			...
			auto tilePosition = position + base::Vector2{ tileBbox.X(), tileBbox.Y() };
