...
void BigFlameEffect::Draw(SpriteExtensions& spriteBatch)
{
	if (!IsFinished())
	{
		...
		spriteBatch.Draw(*flame, base::Vector2{ position.x - flameWidth, position.y - flameHeight });
		spriteBatch.Draw(*flame, base::Vector2{ position.x             , position.y - flameHeight });
		spriteBatch.Draw(*flame, base::Vector2{ position.x + flameWidth, position.y - flameHeight });
		spriteBatch.Draw(*flame, base::Vector2{ position.x - flameWidth, position.y });
		spriteBatch.Draw(*flame, base::Vector2{ position.x             , position.y });
		spriteBatch.Draw(*flame, base::Vector2{ position.x + flameWidth, position.y });
	}
}