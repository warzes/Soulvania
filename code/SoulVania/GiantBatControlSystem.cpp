...
bool GiantBatControlSystem::IsDistanceOutsideOfViewport(base::Vector2 distance)
{
	auto position = parent.GetPosition();
...
{
	auto position = parent.GetPosition();
	auto distanceToPlayer = base::Vector2::Distance(position, playerPosition);
...
base::Vector2 GiantBatControlSystem::RandomDirection()
{
	if (IsBelowPlayer())
